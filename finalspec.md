...
* The robot .cpp files are placed into the same directory as the arena, and when the game loads, the arena will compile the robots into .so (shared objects) using the arena's RobotBase.o - this makes sure that there's no cheating. 
...
* The steps that the arena takes when it runs are as follows:
    ...
    4. Loop through each robot in the vector:
        ...
        10. if the robot returns false, call this_robot->get_move and handle the move request. Do NOT call get_move if you handled a shot case. The robots only get one action, move or shoot.
